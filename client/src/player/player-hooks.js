...
export default function usePlayTrack() {
  ...
  const { setIsPlaying, device, setMonitorCurrentlyPlaying } =
...
export function useUpdatePlayingTrack() {
  ...
  const { setTrackId, tracks, setTracks, tracksInitialized } =
    useContext(PlaylistContext);
  const { setIsPlaying, monitorCurrentlyPlaying } = useContext(PlayerContext);
  ...
  const setPlaylistFromContext = useCallback(
    (uri, item) => {
      if (uri.includes(":artist:")) {
        server()
          .get(`/api/playlists/${uri}`)
          .then(({ data }) => {
    ...
    return spotifyAxios()
      ...
      .then(({ data }) => {
        ...
        if (item) {
          ...
          const found =
            tracksInitialized || tracks.find((t) => trackUri === t.uri);
          const { uri } = context || {};
