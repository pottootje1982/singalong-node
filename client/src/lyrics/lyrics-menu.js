...
export default function LyricsMenu({ lyricsRef, trackFilters }) {
  ...
  const { monitorCurrentlyPlaying, setMonitorCurrentlyPlaying } =
  ...
  function downloadAndClose() {
    closeMenu();
    downloadTrack(track).then((lyrics) => {
  ...
  return (
    <>
      ...
      >
        ...
        <Divider />
        <IconMenuItem onClick={saveLyrics} icon="Save" text="Save" />
        <IconMenuItem onClick={removeLyrics} icon="Delete" text="Remove" />
        <Divider />
        <IconMenuItem
        ...
        />
        <CustomSearch setModalOpen={setCustomSearchOpen} />
        <Divider />
        <CheckMenuItem
          setter={setDarkMode}
          checked={darkMode}
