...
export function getPlaylist(location) {
  ...
  const [, urlCurrentlyPlaying] =
    location.pathname.match(/currently-playing\/(.*)/) || [];
  ...
  return {
    ...
    urlCurrentlyPlaying:
      urlCurrentlyPlaying &&
      urlCurrentlyPlaying.replace(/spotify:user:\d+/, "spotify"),
  };
}
...
export function PlaylistProvider(props) {
  ...
  const { urlPlaylist, urlRadio, urlCustomPlaylist, urlCurrentlyPlaying } =
    getPlaylist(location);
  ...
  const [lyricsHeight, setLyricsHeight] = useState(
    parseInt(window.localStorage.getItem("lyricsHeight"))
  );
  ...
  useEffect(
    () => window.localStorage.setItem("lyricsHeight", lyricsHeight),
    [lyricsHeight]
  );
