...
export class ChartLyricsEngine extends LyricsSearchEngine {
  ...
  constructor() {
    super("ChartLyrics", {
      domain: "http://api.chartlyrics.com",
      lyricsLocation: "Lyric",
    });
  }

  public searchLyrics(artist: string, title: string) {
    const downloadUrl = `http://api.chartlyrics.com/apiv1.asmx/SearchLyricDirect?artist=${this.encode(
      artist
    )}&song=${this.encode(title)}`;
