...
function getTrack(trackElement) {
  ...
  return {
    artist: trackElement.attr("data-artist"),
    title: trackElement.attr("data-title"),
    id: trackElement.prop("id")
  }
}
...
function updateDownloadStatus(playlist, sleepTime, index, updateLyrics) {
  return function(result) {
    ...
    if (updateLyrics) $("#lyrics-display").val(track.lyrics)
...
function downloadPlaylistRecursive(playlist, sleepTime, index) {
  ...
  colorTrack(playlist[0].id, "orange")
  ajax(
    "/download-track",
    { track: getTrack(playlist[0]), sleepTime: sleepTime },
    updateDownloadStatus(playlist, sleepTime, index)
  )
}
