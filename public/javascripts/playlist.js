...
function refreshPlaylistControls(res) {
  ...
  if (res.updateTextualPlaylist) $("#playlistText").val(res.textualPlaylist)
  if (res.playlistHtml) $("#playlist").replaceWith(res.playlistHtml)
  var name
  ...
  else name = $("#currently-playing-link").text()
...
function refreshPlaylist(res) {
  ...
  if (res.hasMore) ajax("/playlist", res, refreshPlaylist)
  else
